...
$(document).ready(function() {								
	...
	$('select[name="deppart"]').change(function ()
	{
		$.ajax({
			 type: "post",
			 url: "AjaxVille2.php",
			 data: "deppart="+$('select[name="deppart"]').val(),
			 success: function(msg)
				{			 
					//alert(msg);
					$('select[name="villeville"]').html(msg);
				}
		 });
	});	
...
elseif ( isset($_GET['formRech']) )
{
			...
			echo'
				<div class="con" style="height:150px;padding: 10px 10px 10px 10px;margin:0 0 10px;">
					<div class="container">
						<div class="row">
							<h1>Devis n°'.$i.' :</h1>
							<p><strong>Activité : </strong> '.$activite.'<br />
							<strong>Zone d\'intervention : </strong> '.$zone_intervention.'<br />
							<a href="recherche.php?idDevisEnch='.$res['id_devis'].'">+ Détail</a>
						</div>
					</div>
				</div>
				';
	...
	*/
	...
	if ( $dep==0 && $cat==0 )
	{
		$req = $my->req('SELECT id as id_devis FROM ttre_achat_devis WHERE statut_valid_admin=1 ORDER BY id DESC');
	}
	elseif ( $dep==0 && $cat!=0 )
	{
		$req = $my->req('SELECT * 
					...
					<div class="con" style="height:150px;padding: 10px 10px 10px 10px;margin:0 0 10px;">
						<div class="container">
							<div class="row">
								...
								<strong>Zone d\'intervention : </strong> '.$zone_intervention.'<br />
								<a href="recherche.php?idDevisAImm='.$res['id_devis'].'">+ Détail</a>
							</div>
						</div>
					</div>
					';
...
else
{
	...
	{
		$i=0;
		$select_dep.='<select name="deppart" id="ip_dep"><option value="0"></option> ';
		while ( $resCat=$my->arr($reqCat) )
		{
			//$check_activite.='<input id="ip_act" type="checkbox" name="categorie[]" value="'.$resCat['id'].'" /> '.$resCat['titre'].'<br /> ';
				...
				<div class="row">
					<div class="col-md-12">
						<div class="form-groupe-pro ">
							...
							<p>
								<label for="ip_act" >Activité : </label><br />
								'.$select_activite.'
							</p>
							<p>
								<label for="ip_dep">Département : </label><br />
								'.$select_dep.'
							</p>
							<p>
								<label for="ip_ville">Ville : </label><br />
								<select name="villeville" id="ip_ville"><option value="0"></option>
							</p>
...
?>									 
								...
								<div class="con" style="height:150px;padding: 10px 10px 10px 10px;">
									<div class="container">
										<div class="row">
											<h1>Devis n°1 :</h1>
											<p><strong>Activité : </strong> Lorem ipsum dolor sit amet<br />
											<strong>Zone d'intervention : </strong> Lorem ipsum dolor sit ametù<br />
											<a href="#">+ Détail</a>
										</div>
									</div>
								</div>
