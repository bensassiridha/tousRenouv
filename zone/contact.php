...
if ( isset($_POST['envoyer_form']) )
{
	...
	else
	{
		...
		$temppp = $my->req_arr('SELECT * FROM ttre_departement_france WHERE departement_id='.$_SESSION['zone'].' ');
		mail($mail_client,'Zone '.$temppp['departement_nom'].' - '.$sujet,$message,$headers);
		
		$tempp = $my->req_arr('SELECT * FROM ttre_zcoordonnees WHERE id='.$_SESSION['user_zone'].' ');
		mail($tempp['email'],$sujet,$message,$headers);
