...
if ( isset($_GET['idDevisAImm']) )
{
	...
	header("location:espace_professionnel.php?contenu=devisa_att_paye&idDevis=".$_GET['idDevisAImm']."&etape=paiement");exit;
}
elseif ( isset($_GET['idDevisEnch']) )
{
	...
	header("location:espace_professionnel.php?contenu=devis_encours&idDevis=".$_GET['idDevisEnch']."");exit;
}
...
if ( isset($_GET['idEstimPaye']) )
{
	...
	{
		header("location:espace_professionnel.php?contenu=devisa_att_paye&paiement=annuler");exit;
	}
}
...
if ( isset($_GET['contenu']) )
{
	switch ( $_GET['contenu'] )
	{
		case 'connexion' :
			...
			{
				$req=$my->req('SELECT * FROM ttre_client_pro WHERE email="'.$_POST['email_connexion'].'" AND mdp="'.md5($_POST['mdp_connexion']).'" ');
				if ( $my->num($req)==0 )
				{
					header("location:espace_pro.php?inscrit=erreur");exit;
				}
				else
				{
					$cl=$my->arr($req);
					if ( $cl['stat_valid_zone']==1 OR $cl['stat_valid_general']==1 )
						$_SESSION['id_client_trn_pro']=$cl['id'];
					else
						$_SESSION['id_client_trn_pro_desactiver']=$cl['id'];
		...
		case 'inscription' :
			...
			{
				if ( md5($_POST['validation']) != $_SESSION['valeur_image'] ){header("location:espace_pro.php?contenu=inscription&erreur=validation");exit;}
				else
				{
					$req=$my->req('SELECT * FROM ttre_client_pro WHERE email="'.$_POST['mail_nouveau'].'"');
					if ( $my->num($req)>0 ) {header("location:espace_pro.php?contenu=inscription&erreur=mail");exit;}
					else
					{
						$fich1_nouveau='';
									...
									<html>
										...
										<body style="background:white; margin:10px; font-family:Verdana, Arial, Helvetica, sans-serif; color:#000; font-size:14px;">
											<div id="corps" style="margin:0 auto; width:800px; height:auto;">
												...
												<p>Bonjour,</p>
												<p>Un artisan ou societé "'.$_POST['raison'].'" vient de s\'inscrire. 
						...
						$_SESSION['id_client_trn_pro_desactiver']=$id_client;
						
						if ( isset($_SESSION['lien']) )
							header("location:".$_SESSION['lien']."");
						else
							header("location:espace_professionnel.php?inscrit=enattente");
						exit;
									...
									<html>
										...
										<body style="background:white; margin:10px; font-family:Verdana, Arial, Helvetica, sans-serif; color:#000; font-size:14px;">
											<div id="corps" style="margin:0 auto; width:800px; height:auto;">
												...
												<p>Bonjour,</p>
												<p>Un artisan ou societé "'.$_POST['raison'].'" vient de faire des modifications dans  son espace personnel.       
		...
		case 'devis_att_paye' :
			...
			if ( isset($_GET['etape']) && $_GET['etape']=='paiement' && isset($_GET['module']) )
			{
				...
				elseif ( $_GET['module']=='cheque' ) $type_de_payement='Chéque';
					...
					<div class="professional-space">
						<div class="container">
							...
							<div class="col-md-12">
								...
								<h5>Gérer toutes vos informations personnelles ainsi que vos devis</h5>
	...
	switch ( $_GET['contenu'] )
	{
		case 'inscription' :
			...
			echo'
				'.$alert.'
				<div id="">
					<form name="client_ajout" action="espace_pro.php?contenu=inscription" method="post" enctype="multipart/form-data" >
					<div class="container">
						<div class="row">
							<div class="col-md-12">
						...
						<p>
							<input type="checkbox" name="condition" id="condition"/> Acceptation des <a href="mention.php#prof" target="_blanc" title="Conditions générales"> conditions générales </a>
						</p>
					...
					<p class="margin_top_20"><a href="espace_pro.php">Retour à la page précédente</a></p>
				</div>
				...
				elseif ( isset($_GET['etape']) && $_GET['etape']=='paiement' )
				{
					if ( !isset($_GET['module']) )
					{
						...
						if ( $temp['statut']==1 && !empty($temp['val1']) && !empty($temp['val2']) ) 
						{
							$module_cheque='
											<p class="payment_module">
												<a title="Payer par Chéque" href="espace_professionnel.php?contenu=devis_att_paye&idDevis='.$_GET['idDevis'].'&etape=paiement&module=cheque&idEnchere='.$adcp['id'].'">
					...
					else
					{
						if ( $_GET['module']=='test' )
						{
							echo'
								<div id="espace_compte">
									<a href="espace_professionnel.php?contenu=devis_att_paye&idEncherePaye='.$_GET['idEnchere'].'">Paiement effectué</a> |
						...
						elseif ( $_GET['module']=='paypal' )
						{
							...
							echo'
								<div id="espace_compte">
									<p style="text-align: center"><i>Vous êtes rederigé sur le site de paypal en quelque instant ...</i></p>
									...
									<div style="display:none;">
										<form action="'.$action_form.'" method="post" name="form_paiemant">
											...
											<input alt="Effectuez vos paiements via PayPal : une solution rapide, gratuite et sécurisée" name="submit" 
						...
						elseif ( $_GET['module']=='virement' )
						{
							...
							$tempdevis=$my->req_arr('SELECT * FROM ttre_devis WHERE id='.$_GET['idDevis'].' ');
							$tempdevisclient=$my->req_arr('SELECT * FROM ttre_devis_client_pro WHERE id='.$_GET['idEnchere'].' ');
							//$text_envoie_mail='<br />Un email contenant ces informations vous a été envoyé.';
							$text_envoie_mail='';
							//<br />- à cette banque <span class="bold"> ---- BANQUE ---- </span>
							echo'
								<div id="espace_compte" style="margin:0 0 0 80px;">
									<p>Votre commande sur '.$nom_client.' est bien enregistré.</p>
									<p>Merci de nous envoyer un virement bancaire avec :
										...
										<br />- à l\'ordre de <strong> '.$temp['val2'].' </strong> 
										<br />- suivant ces détails 
										<br />
										IBAN : <strong> '.$temp['val1'].' </strong>
										<br />
										SWIFT BIC : <strong> '.$temp['val3'].' </strong> 
										<br />- N\'oubliez pas d\'indiquer votre référence de commande <strong> "'.$tempdevis['reference'].'" </strong> dans le sujet de votre virement
										'.$text_envoie_mail.'
										<p>Votre commande vous sera validé dès réception du paiement.</p>
										<p>Pour toute question ou information complémentaire merci de contacter notre <a href="contact.php">support client</a></p>.
									</p>
								</div>
								';
						}
						elseif ( $_GET['module']=='cheque' )
						{
							...
							echo'
								<div id="espace_compte" style="margin:0 0 0 80px;">
									<p>Votre commande sur '.$nom_client.' est bien enregistré.</p>
									<p>Merci de nous envoyer un chèque :
										<br />- d\'un total de <strong> '.number_format($tempdevisclient['prix_enchere'],2).' € </strong>
										<br />- à l\'ordre de  <strong> '.$temp['val1'].' </strong> 
										<br />- à envoyer à 
										<br />
										<strong> '.$temp['val2'].' </strong>
										'.$text_envoie_mail.'
										<br />- N\'oubliez pas d\'indiquer votre référence de commande <strong> "'.$tempdevis['reference'].'" </strong> sur le chéque
										<p>Votre commande vous sera validé dès réception du paiement.</p>
										<p>Pour toute question ou information complémentaire merci de contacter notre <a href="contact.php">support client</a></p>.
									</p>
								</div>
				...
				elseif ( isset($_GET['etape']) && $_GET['etape']=='paiement' )
				{
					if ( !isset($_GET['module']) )
					{
						...
						if ( $temp['statut']==1 && !empty($temp['val1']) && !empty($temp['val2']) ) 
						{
							$module_cheque='
											<p class="payment_module">
												<a title="Payer par Chéque" href="espace_professionnel.php?contenu=devisa_att_paye&idDevis='.$_GET['idDevis'].'&etape=paiement&module=cheque&idEstim='.$adcp['id'].'">
													<img width="86" height="49" alt="Payer par Virement" src="stockage_img/cheque.jpg">
													Payer par Chéque 
												</a>
											</p>
					...
					else
					{
						if ( $_GET['module']=='test' )
						{
							echo'
								<div id="espace_compte">
									<a href="espace_professionnel.php?contenu=devisa_att_paye&idEstimPaye='.$_GET['idEstim'].'">Paiement effectué</a> |
		...
		case 'devis_admin' :
			if ( isset($_GET['idDevis']) )
			{
				...
				$detail.='
				<p class="margin_top_20"><a href="espace_professionnel.php?contenu=devis_admin">Retour à la page précédente</a></p>
					';
				echo'
				<div id="espace_compte">
					'.$detail.'
			...
			<div class="col-md-12">
				<div class="col-md-6">
					<div class="Login-pro">
						...
						<form action="espace_pro.php?contenu=connexion" name="client_conn" method="post" >
				...
				<div class="col-md-6">
					<div class="Logup-pro">
						...
						<div class="log-pro">
							...
							<a href="espace_pro.php?contenu=inscription"">Créer un compte</a>
						</div>
					<div>
				</div>		
			</div>
			...
			'.$alert.'
				...
				<div class="col-md-4">
					<div class="espace-box2">
						...
						<h3>Devis en cours d\'enchére</h3>
						<p>Vous pourrez consulter le détail de vos devis en cours d\'enchére et dans votre zone d\'intervention.</p>
				...
				<div class="col-md-4">
					<div class="espace-box2">
						...
						<p>C\'est dans cette section que vous pourrez consulter le détail de vos devis en attende de payement.</p>
				...
				<div class="col-md-4">
					<div class="espace-box2">
						...
						<h3>Devis avec achat immédiat en attende de pay</h3>
						<p>Vous pourrez consulter le détail de vos devis avec achat immédiat en attende de payement.</p>
				...
				<div class="col-md-4">
					<div class="espace-box2">
						...
						<h3>Historique de devis avec achat immédiat</h3>
						<p>C\'est dans cette section que vous pourrez consulter l\'historique de vos devis  avec achat immédiat.</p>
				...
				<div class="col-md-12">
					<div class="get-quote">						
						...
						<div class="quote-btns" style="padding-right:10px;" >
								<a href="devis.php" style="padding:12px 10px;" class="create">Créer votre devis</a>
								<a href="prix-travaux.php" style="padding:12px 10px;" class="get">Devis Immédiat</a>
