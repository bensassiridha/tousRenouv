...
$pageTitle = "Créer Votre Devis | Devis gratuit en ligne et sans aucun engagement"; 
...
if ( $my->num($reqCat)>0 )
{
	...
	while ( $resCat=$my->arr($reqCat) )
	{
		...
		$liCat.='<li><a href="devis.php?nomCat='.suppr_speciauxxx($resCat['titre']).'&cat='.$resCat['id'].'">'.$logo.' <br /> '.$resCat['titre'].'</a></li>';
	}
...
$(document).ready(function() {      
	...
	$('form[name="devis"]').submit(function ()
	{
		...
		else if( $.trim(this.nbr.value)==0 ) { mes_erreur+='<p>Nombre d\'estimation doit être supérieur à 0 !</p>'; }
		if ( mes_erreur ) { $("#note").addClass("error");$("#note").hide().html(mes_erreur).fadeIn("slow"); return(false); }
	});
	/*$('.nbr_estimatin').blur(function ()
	{
		mes_erreur='';
		if( parseInt($(this).val())==0 || $(this).val()=="" ) { mes_erreur+='<p>Nombre doit être supérieur à 0.</p>'; }
