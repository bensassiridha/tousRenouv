...
if ( isset($_GET['contenu']) )
{
	switch ( $_GET['contenu'] )
	{
			...
			if ( isset($_GET['action']) )
			{
				switch ( $_GET['action'] )
				{
					...
					case 'information' :
						if ( !isset($_SESSION['devis_trn']) ) {header("location:devis.php");exit;}
						if ( isset($_SESSION['id_client_trn_part']) ) {header("location:devis.php?contenu=adresse");exit;}
						if ( isset($_POST['submit_info_client']) )
						{
							$my->req("INSERT INTO ttre_client_part VALUES('',
								'".$my->net_input('--')."',
								'".$my->net_input('--')."',
							...
							$idc=mysql_insert_id();
							$_SESSION['id_visite_trn_part']=$idc;
							header("location:devis.php?contenu=adresse&action=information");exit;
						}
						break;
				}
			}
		...
		case 'adresse' :
			...
			if ( !isset($_SESSION['devis_trn']) ) {header("location:devis.php");exit;}
			if ( !isset($_SESSION['id_client_trn_part']) && !isset($_SESSION['id_visite_trn_part']) ) {header("location:devis.php?contenu=authentification");exit;}
            ...
            //echo '<pre>';print_r($_SESSION['adresse_chantier_trn']);echo  '</pre>';
				...
				switch ( $_GET['action'] ) 
				{
					case 'information' :
						if ( isset($_POST['submit_adresse']) )
						{
							...
							$cl=$my->req_arr('SELECT * FROM ttre_client_part WHERE id='.$_SESSION['id_visite_trn_part'].' ');
							$message = '
								<html>
								<head>
								...
								<body style="background:white; margin:10px; font-family:Verdana, Arial, Helvetica, sans-serif; color:#000; font-size:14px;">
									<div id="corps" style="margin:0 auto; width:800px; height:auto;">
										<center><img src="'.$logo_client.'" alt="" /></center><br />
										<h1 style="background-color:#0495CB; color:#FFF; font-size:16px; text-align:center;">'.$nom_client.'</h1>
										<p>Bonjour,</p>
										<p>un nouveau devis a �t� ajout� sur votre base de donn�es par " '.html_entity_decode($cl['nom']).' '.html_entity_decode($cl['prenom']).' ".</p>
							...
							$mail_client='demandes@tousrenov.fr';
		...
		case 'recapitulatif' :
			...
			if ( isset($_POST['envoie_devis']) )
			{
				...
				$message = '
					...
					<body style="background:white; margin:10px; font-family:Verdana, Arial, Helvetica, sans-serif; color:#000; font-size:14px;">
						<div id="corps" style="margin:0 auto; width:800px; height:auto;">
							<center><img src="'.$logo_client.'" alt="'.$resCat['titre'].'" /></center><br />
							<h1 style="background-color:#0495CB; color:#FFF; font-size:16px; text-align:center;">'.$nom_client.'</h1>
							<p>Bonjour,</p>																
							<p>un nouveau devis avec  achat im�diat a �t� ajout� sur votre base de donn�es par " '.html_entity_decode($cl['nom']).' '.html_entity_decode($cl['prenom']).' ".</p> 
							<div id="pied" style="width:800px; height:auto; font-size:14px; margin-top:20px;">
								<p style="padding-top:10px;">'.$nom_client.'</p>											
							</div>
						</div>
...
$pageTitle = "Cr�er Votre Devis | Devis gratuit en ligne et sans aucun engagement"; 
									...
									<h6>Formulaire</h6>
										...
										<?php 
											...
											if ( $my->num($req)>0 )
											{
												...
												{
													echo'<li><a href="upload/fichiers/'.$res['fichier'].'">'.$res['titre'].'</a></li>';
												}
											}
										?>
										</ul>
...
if ( $my->num($reqCat)>0 )
{
	...
	while ( $resCat=$my->arr($reqCat) )
	{
		...
		if ( isset($_GET['cat']) && $_GET['cat']==$resCat['id'] );
		if ( !empty($resCat['photo']) ) $logo='<img src="upload/logosCateg/50X30/'.$resCat['photo'].'" alt="'.$resCat['titre'].'" />'; else $logo='';
		$liCat.='<li><a href="devis.php?nomCat='.suppr_speciauxxx($resCat['titre']).'&cat='.$resCat['id'].'"><div class="category-icon">'.$logo.'</div> <div class="category-title">'.$resCat['titre'].'</div></a></li>';
	}
	echo '<ul id="menu_cat" class="quote-categories">'.$liCat.'</ul>';
}
?>
</div>
</div>
			...
			<form name="cat_ajout" action="devis.php?contenu=panier&action=ajouter" method="post" enctype="multipart/form-data" class="">
						<div class="description col-md-12">
							...
							<p class="align_center padding_tb_20">
								...
								<button type="submit" class="sub" name="ajout_client"><i class="fa fa-paper-plane-o" aria-hidden="true"></i> Valider</button>
...
$(document).ready(function() {      
	...
	$('form[name="devis"]').submit(function ()
	{
		mes_erreur='';
		if( !$.trim(this.nbr.value) ) { mes_erreur+='<p>Il faut entrer le champ Nombre d\'estimation !</p>'; }
		else if( $.trim(this.nbr.value)==0 ) { mes_erreur+='<p>Nombre d\'estimation doit �tre sup�rieur � 0 !</p>'; }
		if ( mes_erreur ) { $("#note").addClass("error");$("#note").hide().html(mes_erreur).fadeIn("slow"); return(false); }
	});
	/*$('.nbr_estimatin').blur(function ()
	{
		mes_erreur='';
		if( parseInt($(this).val())==0 || $(this).val()=="" ) { mes_erreur+='<p>Nombre doit �tre sup�rieur � 0.</p>'; }
	...
	switch ( $_GET['contenu'] )
	{
		case 'authentification' :
				...
				switch ( $_GET['action'] )
				{
						...
						echo'
							...
							<div id="espace_compte" style="margin: 0 0 0 100px;">
								...
								<fieldset>
									...
									<p>
										<input type="checkbox" name="condition" id="condition"/> Acceptation des conditions g�n�rales
									</p>
					...
					case 'information' :
						...
						echo'
							...
							<div class="col-md-12" >
							<form name="client_ajout" action="devis.php?contenu=authentification&action=information" method="post" enctype="multipart/form-data" >
							<div class="container">
								<div class="row">
									<div class="col-md-12">
			...
			else
			{
					...
					<div class="Login-part">
						...
						<form action="espace_particulier.php?contenu=connexion" name="client_conn" method="post" class="" >
							<div class="form-login-part">
									<input id="mail" type="text" name="email_connexion" placeholder="Votre adresse mail" />
									<input id="mdp" type="password" name="mdp_connexion" placeholder="Votre mot de passe"/>
				...
				<div class="col-md-6">
					<div class="Logup-part">
						...
						<div class="log-part">
							...
							<a href="espace_particulier.php?contenu=inscription" class="">Cr�er un compte <i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
						</div>
					</div>
				</div>
				...
				<div class="col-md-6">
					<div class="Logup-part">
						<div class="title-sign-part">Continuez sans inscription</div>
						<div class="log-part">
							<p></p>
							<a href="devis.php?contenu=authentification&action=information" class="">Continuez <i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
						</div>
					</div>
				</div>
					';				
			...
			if ( isset($_GET['action']) )
			{
				switch ( $_GET['action'] ) 
				{
					case 'information' :
						echo'
							<div id="">
								<form  action="devis.php?contenu=adresse&action=information" method="post" enctype="multipart/form-data" class="">
								<div class="container">
									<div class="row">
										<div class="col-md-12">
											<div class="form-groupe-part ">
												<div class="title-info"><h1>Adresse du chantier</h1></div>
													<p>
														<label for="ip_voi">Num�ro et voie : </label>
													...
													<p>
														<label for="ip_cp">Code postal :</label>
														<input id="ip_cp" type="text" name="cp" value="" onKeyPress="return scanTouche(event)"/>
													</p>
													<p>
														<label for="ip_ville">Ville : </label>
														<select id="ip_ville" name="ville"/></select>
													</p>
													<p>
														<label for="ip_pays">Pays :</label>
														<input id="ip_pays" type="text" name="pays" value="France" readonly="readonly" />
													</p>
							
													<p class="align_center padding_tb_20">
														<input value="valider" class="sub" type="submit" name="submit_adresse"/>
													</p>
											</div>
										</div>
									</div>
					...
					case 'ajouter' :
						...
						$contenu.='
							...
							<div id="espace_compte" style="margin: 0 0 0 100px;">
								...
								<p class="margin_top_20"><a href="devis.php?contenu=adresse">Retour � la page pr�c�dente</a></p>
							</div>
							';			
						break;
					case 'modifier' :
						...
						$contenu.='
							...
							<div id="espace_compte" style="margin: 0 0 0 100px;">
								...
								<p class="margin_top_20"><a href="devis.php?contenu=adresse&action=afficher">Retour � la page pr�c�dente</a></p>
							</div>
							';
						break;
					case 'afficher' :
						...
						$contenu.='
								...
								<p class="margin_top_20"><a href="devis.php?contenu=adresse">Retour � la page pr�c�dente</a> 
								| 
			...
			else 
			{
				...
				$contenu.='
					...
					<p class="padding_bottom_20 overflow_hidden">
						...
						<a href="devis.php?contenu=recapitulatif" class="boutons_persos_1 float_right">continuer</a>
					</p>			
						 ';
		...
		case 'recapitulatif' :
				...
				<div id="espace_compte">
					...
					<form name="devis" method="post" enctype="multipart/form-data" class="tpl_form_defaut intitules_moyens champs_larges">
						...
						<fieldset>
							...
							<p>
								<label for="ip_fich6">Nombre d\'estimation : <span class="obli">*</span></label>
								<input id="ip_fich6" type="text" name="nbr"  class="nbr_estimatin" onKeyPress="return scanTouche(event)" value="" />
							</p>
						</fieldset>
				...
				<div id="espace_compte">
					<div class="conteneur_ariane_cmd">
						<ul id="compte_ariane_cmd">
							...
							<li class="courant">4. <a href="devis.php?contenu=recapitulatif">R�capitulatif</a></li>
							<li class="ariane_cmd_fil"></li>
						</ul>
					</div>
				</div><br />
			...
			break;
		case 'envoyer' :
			echo'<br /><br /><br /><div id="note" class="notes"><p>Votre devis a bien �t� enregistr�.</p></div>';
...
elseif ( isset($_SESSION['devis_trn']) )
{
	...
	if ( isset($_GET['supprimer']) ) $alert='<div id="note" class="notes"><p>Produit supprim� avec succ�s.</p></div><br />';
	elseif ( isset($_GET['modifier']) ) $alert='<div id="note" class="notes"><p>Quantit� mise � jour avec succ�s.</p></div><br />';
						...
						<section id="partner">
							<div class="container section4">
								<div class="row">
									<div class="partner">
										<div class="col-md-12">
											<div class="title2" style="margin-top: 60px; margin-bottom: 30px;">
