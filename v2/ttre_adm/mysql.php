<?php
	...
	class mysql
	{
		function mysql()
		{
			//$this->db = 'aroua_ttrenov';
		    //$this->connexion = mysql_connect("localhost","root","");
			$this->connexion = mysqli_connect("localhost","root","", "tousrenovcfr");
			
			$this->select_db = mysqli_select_db($this->connexion, 'tousrenovcfr');			
			return $this->connexion;
		}
		function query($query)
		{
		  $rs_tmp = mysqli_query($query, $this->link) or die("<HR>Echec : ".mysqli_errno()." ".mysqli_error()."<HR>$query<HR>");
		  return($rs_tmp);
		}
		function req($req)
		{
			$this->requete = mysqli_query($this->connexion, $req);
			return $this->requete;
		}
		
		function obj($req)
		{
			$this->objet = mysqli_fetch_object($req);
			return $this->objet;
		}
		
		function arr($req)
		{
			$this->arr = mysqli_fetch_array($req);
			return $this->arr;
		}
...
{$bilel=$arg;
	...
	return $this->arg_net;
		...
		function num($req)
		{
			$this->num_rows = $req?mysqli_num_rows($req):0;
