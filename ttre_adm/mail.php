	...
	{       
		case 'modifier' :
			if ( $_GET['id']==7 )
			{
				if ( isset($_POST['rad7']) )
				{
					$my->req('UPDATE ttre_email SET description = "'.$my->net_input($_POST['rad7']).'" WHERE id='.$_GET['id'].' ');
				...
				else
				{
					...
					$sms=new smsenvoi();
					//$sms->debug=true;
					//echo '--'.$sms->checkCredits()['sms']['LOWCOST'].'--';
					//echo '<pre>';print_r($sms->checkCredits());echo'</pre>';
					
					echo'
						<p>Sms [LOWCOST] restantes : '.$sms->checkCredits()['sms']['LOWCOST'].'</p>
						<p>Sms [PREMIUM] restantes : '.$sms->checkCredits()['sms']['PREMIUM'].'</p>
						';
					$tab_rad=array(1=>'Activer',0=>'Désactiver');
					$alert='';
					if ( isset($_GET['modifier']) ) $alert='<div id="note" class="success"><p>Ce statut a bien été modifié.</p></div>';
					$form = new formulaire('modele_1','?contenu=mail&action=modifier&id='.$_GET['id'],'<h2 class="titre_niv2">'.$cat['titre'].' :</h2>','','','sub','txt','','txt_obl','lab_obl');
					$form->vide($alert);
					$form->radio_cu('Alert sms','rad7',$tab_rad,$cat['description']);
			...
			elseif ( $_GET['id']==8 )
			{
				if ( isset($_POST['csms']) )
				{
					$my->req('UPDATE ttre_email SET description = "'.$my->net_input($_POST['csms']).'" WHERE id='.$_GET['id'].' ');
					rediriger('?contenu=mail&action=modifier&id='.$_GET['id'].'&modifier=ok');
					exit;
				}
				else
				{
					...
					$form->vide($alert);
					$form->text('Contenu sms','csms','style="width:500px;" ','',$cat['description']);
					$form->afficher('Enregistrer les modifications');
				}
			}
			else
...
{
	echo '<h1>G&eacute;rer le contenu email et sms</h1>';
	echo '
