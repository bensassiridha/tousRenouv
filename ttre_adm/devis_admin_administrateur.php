	...
	switch ( $_GET['action'] )
	{
		case 'modifier' :
			if ( isset($_POST['modifier']) )
			{
				...
				rediriger('?contenu=devis_admin_administrateur&stat='.$_GET['stat'].'&modifier=ok');
			}
			else
			{
				if ( isset($_GET['supprfichier']) )
				{
					...
					rediriger('?contenu=devis_admin_ajouter&action=modifier&id='.$_GET['id'].'&fichiersuppr=ok');
				}
				...
				$form = new formulaire('modele_1','?contenu=devis_admin_administrateur&stat='.$_GET['stat'].'&action=modifier&id='.$_GET['id'].'','<h2 class="titre_niv2">Modifier Devis :</h2>','ajouter','','sub','txt','','txt_obl','lab_obl');
...
<!--  				<script type="text/javascript" src="calandar/jquery-1.3.2.js"></script>  -->
				...
				$form->vide('<input type="radio" name="pb"  value="0" > Retour à l\'étape précedente<br /><br /><br />');
				$form->vide('</td></tr>');
				...
				echo '<p><a href="?contenu=devis_admin_administrateur&stat='.$_GET['stat'].'">Retour</a></p>';
		...
		case 'archifier' :
			...
			rediriger('?contenu=devis_admin_administrateur&stat='.$_GET['stat'].'&supprimer=ok');
		break;	
		case 'changer' :
			$info_devis = $my->req_arr('SELECT * FROM ttre_achat_devis WHERE id='.$_GET['id'].' ' );
			...
			$my->req('DELETE FROM ttre_achat_devis_suite WHERE id_devis='.$_GET['id'].' ');
			rediriger('?contenu=devis_admin_administrateur&changer=ok');exit;
			break;
	}
}
else
{
	...
	<!--  				<script type="text/javascript" src="calandar/jquery-1.3.2.js"></script>  -->
				...
				<?php 
	$form = new formulaire('modele_1','?contenu=devis_admin_administrateur&stat='.$_GET['stat'].'','','','','sub','txt','','txt_obl','lab_obl');
	...
	elseif ( isset($_GET['supprimer']) ) echo '<div class="success"><p>Ce devis a bien été archivé.</p></div>';
		...
		if ( $my->num($req)>0 )
		{
			...
			$entete='
					<table id="liste_produits">
						...
						<tbody> 
				';
			$affich_ajout='';$affich_zone='';$affich_admin='';$affich_ajout_zone='';
			while ( $ress=$my->arr($req) )
			{
				...
				if ( $my->num($rq1)>0 || $use_zo==0 )
				{
					...
					{
						if ( $res_suite['stat_devis_attente']==0 ) $affich_aza=$u['nom'].'(ajout) / '.$az['nom'].'(zone)';
					...
					$ligne='
						<tr>
							...
							<td class="bouton">
								<a href="?contenu=devis_admin_administrateur&stat='.$_GET['stat'].'&action=modifier&id='.$res['id'].'" target="_blanc">
							...
							<td class="bouton">
								...
								{window.location=\'?contenu=devis_admin_administrateur&stat='.$_GET['stat'].'&action=archifier&id='.$res['id'].'\'}" title="Archiver">
								Archiver</a>
								<br /><br /><a style="color:red;" href="?contenu=devis_admin_administrateur&action=changer&id='.$res['id'].'" title="Achat immédiat">Changer</a>
							</td>
						</tr>
						';
						/*	<td class="bouton">
					...
					if ( $res_suite['stat_ajout_zone']==1 ) $affich_ajout.=$ligne;
					elseif ( $res_suite['stat_ajout_zone']==2 ) $affich_admin.=$ligne;
					elseif ( $res_suite['stat_ajout_zone']==0 )
					{
						if ( $res_suite['stat_devis_attente']==0 )  $affich_ajout_zone.=$ligne;
						else $affich_zone.=$ligne;
					}
			...
			if ( $affich_ajout!='' )
			{
				echo $entete.$affich_ajout.'</tbody></table><br /><br />';
			}			
			if ( $affich_zone!='' )
			{
				echo $entete.$affich_zone.'</tbody></table><br /><br />';
			}
			if ( $affich_ajout_zone!='' )
			{
				echo $entete.$affich_ajout_zone.'</tbody></table><br /><br />';
			}
			if ( $affich_admin!='' )
			{
				echo $entete.$affich_admin.'</tbody></table><br /><br />';
			}			
