...
if ( !empty($_GET['action']) )
{
	switch ( $_GET['action'] )
	{
		case 'modifier_diap' :
			...
			{
				$req = $my->req('SELECT * FROM ttre_diaporama WHERE id_user='.$_SESSION['id_user'].' AND idg='.$_GET['id'].' ORDER BY id ASC');
				if ( $my->num($req)>0 )
				{
					$i=0;
					...
					{
						$i++;
						$my->req('UPDATE ttre_diaporama SET titre = "'.$_POST['titre'.$i.''].'" , lien = "'.$_POST['Lien'.$i.''].'" WHERE id='.$res['id'].'' );
						$image = $res['photo'];
				...
				for ($i = 1; $i <= $_POST['nbrPhoto']; $i++)
				{
					...
					if ($handle->uploaded)
					{
						...
						if ($handle->processed)
						{
							$image  = $handle->file_dst_name ;	          // Destination file name
							$handle-> Clean();                           // Deletes the uploaded file from its temporary location
							$my->req('INSERT INTO ttre_diaporama VALUES("","'.$_GET['id'].'","'.$_SESSION['id_user'].'","'.$_POST['titre_'.$i.''].'","'.$image.'","'.$_POST['Lien_'.$i.''].'")');
						}
					}
				}
				rediriger('?contenu=zdiaporama&action=modifier_diap&id='.$_GET['id'].'&modifier=ok');
			}
			else
			{
				if ( isset($_GET['supprgal']) )
				{
					...
					rediriger('?contenu=zdiaporama&action=modifier_diap&id='.$_GET['id'].'&photosuppr=ok');
				}
					...
					$photo_existe.='
									<table>
										<tr style="background-color:#D3DCE3;">
											<th align="center"><label>Lien / Titre</label></th>
						...
						$photo_existe.='
						 			<tr style="background-color:#E5E5E5;">
										<td style="text-align: center;">
											<input type="text" name="Lien'.$j.'" value="'.$res['lien'].'"  /> <br />
											<input type="text" name="titre'.$j.'" value="'.$res['titre'].'"  />
										</td>
										<td style="text-align: center;">
										...
										<td align="center">
											<a class="lien_1" href="#" onclick="if(confirm(\'Etes vous certain de vouloir supprimer cette photo ?\'))
												{window.location=\'?contenu=zdiaporama&action=modifier_diap&id='.$_GET['id'].'&supprgal='.$res['id'].'\'}" class="lien_1">
												Supprimer
											</a>
										</td>
									</tr>
				...
				$form = new formulaire('modele_2','?contenu=zdiaporama&action=modifier_diap&id='.$_GET['id'].'','','modifier','','sub','txt','','txt_obl','lab_obl');
							...
							<tr>
								...
								<td>
									...
									<table style="text-align:center;" id="tblPhoto" >
										...
										<tr style="background-color:#E5E5E5;">
											<td>1</td>
											<td><input type="text" name="Lien_1" /><br /><input type="text" name="titre_1" /></td>
				...
				echo '<p><a href="?contenu=zdiaporama">Retour</a></p>';
			}
			break;
	}
}
	...
	echo'
		<table id="liste_produits">
			...
			<tr>
				<td>Diaporama</td>
				<td class="bouton">
					<a href="?contenu=zdiaporama&action=modifier_diap&id=1">
...
function addRowToTablePhoto()
{
	...
	var cellRightSel = row.insertCell(1);
  	var el=$('<input type="text" name="Lien_'+iteration+'" /><br /><input type="text" name="titre_'+iteration+'" />');
