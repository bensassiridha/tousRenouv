	...
	switch ( $_GET['action'] )
	{
		case 'ajouterexel' :
			...
			else
			{
				...
				$form->vide('<tr><td colspan="2">Template ( télécharger et le remplir ) : <a href="template.xls" target="_blanc">template.xls</a></td></tr>');
		...
		case 'modifier' :
			if ( isset($_POST['modifier']) )
			{
				...
				rediriger('?contenu=devis_admin_ajouter&modifier=ok');
			}
			else
			{
				...
				$form = new formulaire('modele_1','?contenu=devis_admin_ajouter&action=modifier&id='.$_GET['id'].'','<h2 class="titre_niv2">Modifier Devis :</h2>','ajouter','','sub','txt','','txt_obl','lab_obl');
				$form->vide('<div id="note"></div>');
...
<!--  				<script type="text/javascript" src="calandar/jquery-1.3.2.js"></script>  -->
		...
		case 'archifier' :
			...
			rediriger('?contenu=devis_admin_ajouter&supprimer=ok');
			break;	
		case 'valider' :
			...
			if ( $my->num($req)>0 )
			{
				...
				while ( $res=$my->arr($req) )
				{
					...
					if ( $tab_devis===$c && $temp['stat_valid']==1 )
					{
...
<!--  				<script type="text/javascript" src="calandar/jquery-1.3.2.js"></script>  -->
				...
				$formsuitezone='
						<p><br />Pb devis :<br />
							...
							<input type="radio" name="pb" '.$pb8.' value="8" >Projet abandonné<br />
		...
		case 'changer' :
			...
			$my->req('DELETE FROM ttre_achat_devis_suite WHERE id_devis='.$_GET['id'].' ');
			rediriger('?contenu=devis_admin_ajouter&changer=ok');exit;
			break;
	}
}
else
{
	echo '<h1>Devis en attente de traitement</h1>';
	...
	$form = new formulaire('modele_1','?contenu=devis_admin_ajouter','','','','sub','txt','','txt_obl','lab_obl');
	...
	echo '<p>Pour ajouter un autre devis à partir d\'un fichier exel , cliquer <a href="?contenu=devis_admin_ajouter&action=ajouterexel">ICI</a></p>';
				...
				if ( $affich==1 )
				{
					...
					echo'
						<tr>
							...
							<td class="bouton">
								<a href="?contenu=devis_admin_ajouter&action=detail&id='.$res['id'].'" target="_blanc">
							...
							<td class="bouton">
								...
								{window.location=\'?contenu=devis_admin_ajouter&action=archifier&id='.$res['id'].'\'}" title="Supprimer">
					...
					if ( $affich==1 )
					{
						...
						echo'
							<tr>
								...
								<td class="bouton">
									<a href="?contenu=devis_admin_ajouter&action=modifier&id='.$res['id'].'" target="_blanc">
									<img src="img/interface/btn_modifier.png" alt="Modifier" /></a>
								</td>
								<td class="bouton">
									<a href="#" onclick="if(confirm(\'Etes vous certain de vouloir archiver ce devis ?\')) 
									{window.location=\'?contenu=devis_admin_ajouter&action=archifier&id='.$res['id'].'\'}" title="Archiver">
									Archiver</a>
									<br /><br /><a style="color:red;" href="?contenu=devis_admin_ajouter&action=changer&id='.$res['id'].'" title="Achat immédiat">Changer</a>
								</td>
							</tr>
							';
							/*	<td class="bouton">
	...
	elseif ( $user['profil']==2 || $user['profil']==6)
	{
		if ( $dep==0 && $cat==0 )
		{
			$req = $my->req('SELECT AD.id as idad
			...
			while ( $ress=$my->arr($req) )
			{
				...
				if ( $affich==1 )
				{
					...
					if ( $my->num($rq1)>0 )
					{
						...
						$respb=$my->req_arr('SELECT * FROM ttre_achat_devis_suite WHERE id_devis='.$res['id'].' ');$pb='';$p='';$d='';
						list($p,$d) = split('[|]',$respb['stat_devis_attente']);
						if ( $p==1 ) { list($d1,$d11) = split('[$]',$d); $pb='Rdv pris le '.$d1.' à '.$d11; }
						elseif ( $p==2 ) { list($d2,$d22) = split('[$]',$d); $pb='A rappeler verifiez à la date '.$d2.' à '.$d22; }
						elseif ( $p==3 ) { $pb='Travaux fini'; }
						elseif ( $p==4 ) { $pb='Faut numéro'; }
						elseif ( $p==5 ) { $pb='Déjà trouvé un artisan'; }
						elseif ( $p==6 ) { $pb='Autre ( '.$d.' )'; }
