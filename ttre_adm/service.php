...
//					$handle->image_ratio_no_zoom_in = true;
				...
				rediriger('?contenu=service&ajouter=ok');
			}
			else
			{
				$form = new formulaire('modele_1','?contenu=service&action=ajouter','<h2 class="titre_niv2">Ajouter service :</h2>','ajouter','','sub','txt','','txt_obl','lab_obl');
				...
				echo '<p><a href="?contenu=service">Retour</a></p>';
			}
			break;
		case 'modifier' :
...
//					$handle->image_ratio_no_zoom_in = true;
				...
				$my->req('UPDATE ttre_service SET 
									...
									photoi		=	"'.$imagei.'" 
								WHERE id = '.$_GET['id'].' ');				
				rediriger('?contenu=service&action=modifier&id='.$_GET['id'].'&modifier=ok');
			}
			else
			{
				if (isset($_GET['supprphoto'] ))
				{
					...
					rediriger('?contenu=service&action=modifier&id='.$_GET['id'].'&photosuppr=ok');
					exit;
				}
				...
				if ( !empty($temp['photo']) )
				{
					...
					$supprimerphoto='
							<a class="lien_1" href="#" onclick="
								...
								{window.location=\'?contenu=service&action=modifier&supprphoto=1&id='.$temp['id'].'\'}">
								Supprimer
							</a>
					';
				}
				...
				if ( !empty($temp['photoi']) )
				{
					...
					$supprimerphotoi='
							<a class="lien_1" href="#" onclick="
								if(confirm(\'Etes vous certain de vouloir supprimer cette photo ?\')) 
								{window.location=\'?contenu=service&action=modifier&supprphotoi=1&id='.$temp['id'].'\'}">
								Supprimer
							</a>
					';
				}
				
				$form = new formulaire('modele_1','?contenu=service&action=modifier&id='.$_GET['id'].'','<h2 class="titre_niv2">Modifier service :</h2>','modifier','','sub','txt','','txt_obl','lab_obl');
				$form->vide($alert);
		...
		case 'supprimer' :
			...
			rediriger('?contenu=service&supprimer=ok');
			break;	
	}
}
else
{
	...
	echo '<p>Pour ajouter une autre service, cliquer <a href="?contenu=service&action=ajouter">ICI</a></p>';
	$req = $my->req('SELECT * FROM ttre_service ');
	if ( $my->num($req)>0 )
	{
		echo'
		...
		while ( $res=$my->arr($req) )
		{
			...
			echo'
				<tr>
					...
					<td>'.$nom.'</td>
					<td class="bouton">
						<a href="?contenu=service&action=modifier&id='.$res['id'].'">
					...
					<td class="bouton">
						...
						{window.location=\'?contenu=service&action=supprimer&id='.$res['id'].'\'}" title="Supprimer">
