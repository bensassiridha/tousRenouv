...
//					$handle->image_ratio_no_zoom_in = true;
				...
				rediriger('?contenu=zannonce&ajouter=ok');
			}
			else
			{
				$form = new formulaire('modele_1','?contenu=zannonce&action=ajouter','<h2 class="titre_niv2">Ajouter Annonce :</h2>','ajouter','','sub','txt','','txt_obl','lab_obl');
				...
				echo '<p><a href="?contenu=zannonce">Retour</a></p>';
			}
			break;
		case 'modifier' :
...
//					$handle->image_ratio_no_zoom_in = true;
				...
				$my->req('UPDATE ttre_annonces SET 
									...
									fichier		=	"'.$fichier.'"
								WHERE id = '.$_GET['id'].' ');				
				rediriger('?contenu=zannonce&action=modifier&id='.$_GET['id'].'&modifier=ok');
			}
			else
			{
				if (isset($_GET['supprphoto'] ))
				{
					...
					rediriger('?contenu=zannonce&action=modifier&id='.$_GET['id'].'&photosuppr=ok');
					exit;
				}
				if (isset($_GET['supprfichier'] ))
				{
					...
					rediriger('?contenu=zannonce&action=modifier&id='.$_GET['id'].'&fichiersuppr=ok');
					exit;
				}
				...
				if ( !empty($temp['photo']) )
				{
					...
					$supprimerphoto='
							<a class="lien_1" href="#" onclick="
								...
								{window.location=\'?contenu=zannonce&action=modifier&supprphoto=1&id='.$temp['id'].'\'}">
								Supprimer
							</a>
					';
				}
				...
				if ( !empty($temp['fichier']) )
				{
					...
					$supprimerfichier='
							<a class="lien_1" href="#" onclick="
								...
								{window.location=\'?contenu=zannonce&action=modifier&supprfichier=1&id='.$temp['id'].'\'}">
								Supprimer
							</a>
					';
				}
				
				$form = new formulaire('modele_1','?contenu=zannonce&action=modifier&id='.$_GET['id'].'','<h2 class="titre_niv2">Modifier annonce :</h2>','modifier','','sub','txt','','txt_obl','lab_obl');
				$form->vide($alert);
...
else
{
	...
	echo '<p>Pour ajouter une autre annonce, cliquer <a href="?contenu=zannonce&action=ajouter">ICI</a></p>';
	$req = $my->req('SELECT * FROM ttre_annonces WHERE id_user='.$_SESSION['id_user'].' ORDER BY ordre ASC');
	if ( $my->num($req)>0 )
	{
		echo'
			<form method="POST" action="?contenu=zannonce&action=ordre" >
		...
		while ( $res=$my->arr($req) )
		{
			echo'
				<tr>
					...
					<td class="bouton">
						<a href="?contenu=zannonce&action=modifier&id='.$res['id'].'">
					...
					<td class="bouton">
						...
						{window.location=\'?contenu=zannonce&action=supprimer&id='.$res['id'].'\'}" title="Supprimer">
