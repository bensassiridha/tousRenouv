	...
	switch ( $_GET['contenu'] )
	{
		case 'connexion' :
			if ( isset($_POST['conn_client']) )
			{
				...
				if ( $my->num($req)==0 )
				{
					rediriger("espace_affilier.php?inscrit=erreur");exit;
				}
				else
				{
					$cl=$my->arr($req);
					...
					rediriger("espace_affilier.php");exit;
				}
			}
			break;
		case 'inscription' :
			...
			{
				if ( md5($_POST['validation']) != $_SESSION['valeur_image'] ){rediriger("espace_affilier.php?contenu=inscription&erreur=validation");exit;}
				else
				{
					$req=$my->req('SELECT * FROM ttre_users WHERE login="'.$_POST['mail_nouveau'].'"');
					if ( $my->num($req)>0 ) {rediriger("espace_affilier.php?contenu=inscription&erreur=mail");exit;}
					else
					{
						...
						rediriger("espace_affilier.php?inscrit=enattente");exit;			
					}
				}
			}
			break;
		case 'modif_param' :
			if ( !isset($_SESSION['id_client_trn_affil']) ) {rediriger("espace_affilier.php");exit;}
			if ( isset($_POST['modif_client']) )
			{
				if ( md5($_POST['validation']) != $_SESSION['valeur_image'] ){rediriger("espace_affilier.php?contenu=modif_param&erreur=validation");exit;}
				else
				{
					$req=$my->req('SELECT * FROM ttre_users WHERE login="'.$_POST['mail'].'" AND id!='.$_SESSION['id_client_trn_affil'].' ');
					if ( $my->num($req)>0 ) {rediriger("espace_affilier.php?contenu=modif_param&erreur=mail");exit;}
					else
					{
						...
						if ( isset($_POST['partenaire']) )
						{
							$req_part=$my->req('SELECT * FROM ttre_partenaire_affi WHERE email="'.$_POST['mail'].'" ');
							if ( $my->num($req_part)==0 ) $my->req("INSERT INTO ttre_partenaire_affi VALUES('','".$my->net_input($_POST['mail'])."') ");
						}
						else
						{
							$my->req('DELETE FROM ttre_partenaire_affi WHERE email="'.$_POST['mail'].'" ');
						}
						
						unset ($_SESSION['id_client_trn_affil']);
						rediriger("espace_affilier.php?modif=enattente");exit;	
					}
				}
			}
			break;
		case 'modif_mdp' :
			...
			if ( isset($_POST['modif_mdp']) )
			{
				...
				if ( $can['password']!=md5($_POST['pass']) ){rediriger("espace_affilier.php?contenu=modif_mdp&erreur=mdp");exit;}
				else
				{
					$my->req('UPDATE ttre_users SET password="'.md5($_POST['pass1']).'" WHERE id_user='.$_SESSION['id_client_trn_affil'].'') ;
					rediriger("espace_affilier.php?modif_mdp=ok");exit;
				}				
			}
			break;
		case 'mdp_perdu' :
			...
			if ( isset($_POST['mdp_perdu']) )
			{
				...
				if ( $my->num($rec)==0 ){rediriger("espace_affilier.php?contenu=mdp_perdu&erreur=mail");exit;}
				else
				{
					$can=$my->arr($rec);
		...
		case 'devis' :
			...
			if ( isset($_GET['action']) && $_GET['action']=='ajouter' && isset($_POST['ajouter']) )
			{
				...
				//-----------------------------------------------------------
				rediriger("espace_affilier.php?contenu=devis&ajouter=ok");exit;
			...
			elseif ( isset($_GET['action']) && $_GET['action']=='supprimer' )
			{
				...
				rediriger("espace_affilier.php?contenu=devis&supprimer=ok");exit;
			}
			break;	
		case 'probleme' :
			if ( isset($_GET['action']) && $_GET['action']=='detail' )
			{
				if ( isset($_POST['modif_comm_stat']) )
				{
					...
					rediriger("espace_affilier.php?contenu=probleme&stat=".$_GET['stat']."&modifier=ok");exit;
				}
			}
			elseif ( isset($_GET['action']) && $_GET['action']=='supprimer' )
			{
				$my->req('UPDATE ttre_achat_devis SET stat_suppr="1" WHERE id='.$_GET['id']);
				rediriger("espace_affilier.php?contenu=probleme&stat=".$_GET['stat']."&supprimer=ok");exit;
			}
			break;	
		case 'deconnexion' :
...
$pageTitle = "Espace Affilié | Devis gratuit en ligne et sans aucun engagement"; 
					...
					<div class="container">
						...
						<div class="form-groupe-part ">
							<div class="title-info"><h1>Informations personnelles</h1></div>
								...
								<p>
									<input type="checkbox" name="condition" id="condition"/> Acceptation des <a href="mentionAffi.php" target="_blanc" title="Conditions générales"> conditions générales </a>
								</p>
