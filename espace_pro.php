	...
	switch ( $_GET['contenu'] )
	{
		case 'connexion' :
			if ( isset($_POST['conn_client']) )
			{
				...
				if ( $my->num($req)==0 )
				{
					rediriger("espace_pro.php?inscrit=erreur");exit;
				}
				else
				{
					$cl=$my->arr($req);
		...
		case 'inscription' :
			...
			{
				if ( md5($_POST['validation']) != $_SESSION['valeur_image'] ){rediriger("espace_pro.php?contenu=inscription&erreur=validation");exit;}
				else
				{
					$req=$my->req('SELECT * FROM ttre_client_pro WHERE email="'.$_POST['mail_nouveau'].'"');
					if ( $my->num($req)>0 ) {rediriger("espace_pro.php?contenu=inscription&erreur=mail");exit;}
					else
					{
						$fich1_nouveau='';
						...
						if ( isset($_SESSION['lien']) )
							...
							rediriger("espace_professionnel.php?inscrit=enattente");
						exit;
