	...
	switch ( $_GET['contenu'] )
	{
		case 'connexion' :
			...
			if ( isset($_POST['conn_client']) )
			{
				...
				if ( $my->num($req)==0 )
				{
					rediriger('espace_particulier.php?inscrit=erreur');
					exit;
				}
				else
				{
					...
					rediriger('espace_particulier.php');
					exit;
				}
			}
		...
		case 'inscription' :
			...
			{
				if ( md5($_POST['validation']) != $_SESSION['valeur_image'] ){rediriger("espace_particulier.php?contenu=inscription&erreur=validation");exit;}
				else
				{
					$req=$my->req('SELECT * FROM ttre_client_part WHERE email="'.$_POST['mail_nouveau'].'"');
					if ( $my->num($req)>0 ) {rediriger("espace_particulier.php?contenu=inscription&erreur=mail");exit;}
					else
					{
						...
						{
							$req_part=$my->req('SELECT * FROM ttre_partenaire_part WHERE email="'.$_POST['mail_nouveau'].'" ');
							if ( $my->num($req_part)==0 ) $my->req("INSERT INTO ttre_partenaire_part VALUES('','".$my->net_input($_POST['mail_nouveau'])."') ");
						}
						else 
						{
							$my->req('DELETE FROM ttre_partenaire_part WHERE email="'.$_POST['mail_nouveau'].'" ');
						}
						...
						rediriger("espace_particulier.php?inscrit=enattente");exit;			
					}
				}
			}
			break;
		case 'valider' :
			...
			rediriger("espace_particulier.php?inscrit=valider");exit;	
			break;
		case 'modif_param' :
			if ( !isset($_SESSION['id_client_trn_part']) ) {rediriger("espace_particulier.php");exit;}
			if ( isset($_POST['modif_client']) )
			{
				if ( md5($_POST['validation']) != $_SESSION['valeur_image'] ){rediriger("espace_particulier.php?contenu=modif_param&erreur=validation");exit;}
				else
				{
					$req=$my->req('SELECT * FROM ttre_client_part WHERE email="'.$_POST['mail'].'" AND id!='.$_SESSION['id_client_trn_part'].' ');
					if ( $my->num($req)>0 ) {rediriger("espace_particulier.php?contenu=modif_param&erreur=mail");exit;}
					else
					{
						$referenceValid = uniqid('R');
						...
						if ( isset($_POST['partenaire']) )
						{
							$req_part=$my->req('SELECT * FROM ttre_partenaire_part WHERE email="'.$_POST['mail'].'" ');
							if ( $my->num($req_part)==0 ) $my->req("INSERT INTO ttre_partenaire_part VALUES('','".$my->net_input($_POST['mail'])."') ");
						}
						else 
						{
							$my->req('DELETE FROM ttre_partenaire_part WHERE email="'.$_POST['mail'].'" ');
						}
						...
						rediriger("espace_particulier.php?modif=enattente");exit;	
					}
				}
			}
			break;
		case 'modif_mdp' :
			...
			if ( isset($_POST['modif_mdp']) )
			{
				...
				if ( $can['mdp']!=md5($_POST['pass']) ){rediriger("espace_particulier.php?contenu=modif_mdp&erreur=mdp");exit;}
				else
				{
					$my->req('UPDATE ttre_client_part SET mdp="'.md5($_POST['pass1']).'" WHERE id='.$_SESSION['id_client_trn_part'].'') ;
					rediriger("espace_particulier.php?modif_mdp=ok");exit;
				}				
			}
			break;
		case 'adresse' :
				...
				switch ( $_GET['action'] )
				{
					case 'ajouter' :
						if ( isset($_POST['ajout_adresse']) )
						{
							...
							rediriger("espace_particulier.php?contenu=adresse&ajout_adress=ok");exit;
						}
						break;
					case 'modifier' :
						if ( isset($_POST['modif_adresse']) )
						{
							...
							rediriger("espace_particulier.php?contenu=adresse&modif_adress=ok");exit;
						}
						break;
					case 'supprimer' :
						$r=$my->req('SELECT * FROM ttre_devis WHERE id_adresse='.$_GET['id'].' ');
						...
						rediriger("espace_particulier.php?contenu=adresse&suppr_adress=ok");exit;
						break;
				}
			}	
			break;	
		case 'mdp_perdu' :
			...
			if ( isset($_POST['mdp_perdu']) )
			{
				...
				if ( $my->num($rec)==0 ){rediriger("espace_particulier.php?contenu=mdp_perdu&erreur=mail");exit;}
				else
				{
					$can=$my->arr($rec);
						...
						<body style="background:white; margin:10px; font-family:Verdana, Arial, Helvetica, sans-serif; color:#0066CC; font-size:14px;">
							...
							<div id="corps" style="width:800px; height:auto;">
								...
								<div id="contenu-corps" style="background-color:#E6E6E6; text-align:center; font-size:14px; padding:10px;">
									...
									<br />
										Si vous avez demand� cette modification, veuillez cliquer sur le lien pour valider votre nouveau mots de passe : <br /> <a href="'.$url_site_client.'/espace_particulier.php?contenu=mdp_perdu&ref='.$referencePass.'">Changement de mots de passe</a>.
									</p>														
								</div>
							</div>
					...
					rediriger("espace_particulier.php?contenu=mdp_perdu&mdpperdu=ok");exit;
				}
			}
			elseif ( isset($_GET['ref']) )
			{
				$ligne=$my->req('SELECT * FROM ttre_client_part_generation_pass WHERE cgp_reference="'.$_GET['ref'].'"');
				if ( $my->num($ligne)==0 ) {rediriger("espace_particulier.php?contenu=mdp_perdu&erreur=mdp");exit;}
				else
				{
					$forget=$my->arr($ligne);
					$req_modif = $my->req ( 'UPDATE ttre_client_part SET mdp="'.$my->net_input($forget['cgp_mdp']).'" WHERE id = "'.$forget['cgp_client_id'].'"' );
					rediriger("espace_particulier.php?valid_mdp=ok");exit;
				}				
			}
			break;	
		case 'devis_histo' :
			...
			if ( isset($_POST['envoie_note']) )
			{
				...
				rediriger("espace_particulier.php?contenu=devis_histo&idDevis=".$_GET['idDevis']."&enregistrer=ok");exit;
			}
			if ( isset($_POST['envoie_message']) )
			{
				...
				rediriger("espace_particulier.php?contenu=devis_histo&idDevis=".$_GET['idDevis']."&envoyer=ok");exit;
			}
			break;
		case 'devisa_encours' :
			if ( !isset($_SESSION['id_client_trn_part']) ) {rediriger("espace_particulier-.php");exit;}
				...
				if ( isset($_GET['action']) )
				{
					switch ( $_GET['action'] )
					{
						case 'supprimer' :
							...
							rediriger("espace_particulier.php?contenu=devisa_encours&supprimer=ok");exit;
							break;
						case 'valider' :
							...
							rediriger("espace_particulier.php?contenu=devisa_encours&valider=ok");exit;
							break;
					}
				}
			}
		...
		case 'devisa_histo' :
			...
			if ( isset($_POST['envoie_note']) )
			{
				...
				rediriger("espace_particulier.php?contenu=devisa_histo&idDevis=".$_GET['idDevis']."&enregistrer=ok");exit;
			}
			if ( isset($_POST['envoie_message']) )
			{
				...
				$message = '
								...
								';
				//$mail_client='bilelbadri@gmail.com';
				$sujet = $nom_client.' : Nouveau message du devis avec achat imm�diat de r�f�rence "'.$dd['reference'].'"';
				$headers = "From: \" ".$nom." \"<".$mail.">\n";
				$headers .= "Reply-To: ".$mail_client."\n";
				$headers .= "Content-Type: text/html; charset=\"iso-8859-1\"";
				mail($mail,$sujet,$message,$headers);
				rediriger("espace_particulier.php?contenu=devisa_histo&idDevis=".$_GET['idDevis']."&envoyer=ok");exit;
			}
			break;
		case 'deconnexion' :
			unset ($_SESSION['id_client_trn_part']);
			unset ($_SESSION['panier_trn']);
			rediriger("espace_particulier.php");exit;
			break;	
	}
}
// ----------------------------------------------------pour varAriane ----------------------------
if ( isset($_GET['contenu']) )
{
	switch ( $_GET['contenu'] )
	{
		...
		case 'devisa_encours' :
			if ( isset($_GET['idDevis']) )
				$varAriane = '<span class="home"><a href="espace_particulier.php">Mon compte</a></span> > <a href="espace_particulier.php?contenu=devisa_encours">Gestion de devis avec achat imm�diat</a> > <span class="courant">D�tail</span>';
			else
				$varAriane = '<span class="home"><a href="espace_particulier.php">Mon compte</a></span> > <span class="courant">Gestion de devis avec achat imm�diat</span>';
			break;	
		case 'devisa_histo' :
			if ( isset($_GET['idDevis']) )
				$varAriane = '<span class="home"><a href="espace_particulier.php">Mon compte</a></span> >
...
$pageTitle = "Espace Particulier | Devis gratuit en ligne et sans aucun engagement"; 
					...
					<div class="particulier-space">
						<div class="container">
							...
							<div class="col-md-12">
								...
								<h5>G�rer toutes vos informations personnelles ainsi que vos commandes</h5>
	...
	switch ( $_GET['contenu'] )
	{
		case 'inscription' :
			...
			echo'
				...
				<div class="col-md-12" >
					...
					<div class="container">
						...
						<div class="title-info"><h1>Informations personnelles</h1></div>
							<label for="ip_etes1">Vous �tes : </label><br />
						...
						<!-- <p>
							<label for="ip_etes2">Vous �tes : </label><br />
						...
						<p>
							...
							<label for="ip_prenom">Pr�nom : <span class="obli">*</span></label><br />
							<input id="ip_prenom" type="text" name="prenom_nouveau" placeholder="Pr�nom"/><br />
							<label for="ip_tel">T�l�phone : <span class="obli">*</span></label><br />
							<input id="ip_tel" type="text" name="telephone_nouveau" placeholder="T�l�phone"/><br />
							<label for="ip_voi">Num�ro et voie : <span class="obli">*</span></label><br />
							<input id="ip_voi" type="text" name="numvoie_nouveau" placeholder="Num�ro et voie"/><br />
							...
							<label for="ip_bat">B�timent, R�sidence, Entr�e: <span class="obli">*</span></label><br />
							<input id="ip_bat" type="text" name="bat_nouveau" placeholder="B�timent, R�sidence, Entr�e"/><br />
						...
						<p>
							<input type="checkbox" name="condition" id="condition"/> Acceptation des <a href="mentionPart.php" target="_blanc" title="Conditions g�n�rales"> conditions g�n�rales </a>
						</p>
					...
					<p class="margin_top_20"><a href="espace_particulier.php">Retour � la page pr�c�dente</a></p>
				</div>
				...
				';			
						//	<input type="checkbox" name="condition" id="condition"/> Acceptation des <a class="various1" href="#inline_insc" title="Conditions g�n�rales"> conditions g�n�rales </a>
			break;
		case 'modif_param' :
			...
			$req_part=$my->req('SELECT * FROM ttre_partenaire_part WHERE email="'.$cl['email'].'" ');
					...
					<div class="container">
						<div class="row">
							...
							<div class="form-groupe-part ">
									<div class="title-info"><h1>Informations personnelles</h1></div>
						<p>
							<label for="ip_etes1">Vous �tes : </label>
						...
						<p>
							<label for="ip_etes2">Vous �tes : </label>
						...
						<p>
							<label for="ip_bat">B�timent, R�sidence, Entr�e : </label>
		...
		case 'adresse' :
					...
					case 'ajouter' :
						...
						echo'
							...
							<div id="">
								...
								<p class="margin_top_20"><a href="espace_particulier.php?contenu=adresse">Retour � la page pr�c�dente</a></p>
							</div>
							';			
						break;				
					case 'modifier' :
			...
			{
				$alert='<div id="note"></div><br />';
				if ( isset($_GET['ajout_adress']) )echo'<div id="note" class="notes"><p>Adresse ajout�e.</p></div><br />';
				elseif ( isset($_GET['modif_adress']) )echo'<div id="note" class="notes"><p>Adresses modifi�e.</p></div><br />';
				elseif ( isset($_GET['suppr_adress']) )echo'<div id="note" class="notes"><p>Adresse supprim�e.</p></div><br />';
				$contenu_adresse='';
				...
				$contenu_adresse.='<p class="margin_top_20"><a href="espace_particulier.php">Retour � la page pr�c�dente</a> | <a href="espace_particulier.php?contenu=adresse&action=ajouter">Ajouter une adresse</a></p>';
				echo'
					<div id="espace_compte">
						'.$contenu_adresse.'
					</div>
		...
		case 'devis_histo' :
			if ( isset($_GET['idDevis']) )
			{
				...
				$detail.='
					<li class="col-md-4">
						...
						<dl>
							...
							<dd>Num�ro et voie : '.html_entity_decode($resoo['num_voie']).'</dd>
				...
				</table></div><a id="various3" href="imp_cheque.php?idDevis='.$_GET['idDevis'].'&suite=ok">Aper�u avant imprimer</a>';
				...
				if ( isset($_GET['enregistrer']) ) $alert='<div id="note" style="margin: 10px 0 0 120px;width:400px;" class="success" ><p>Votre note a bien �t� enregistr�.</p></div>';
				...
				<p class="margin_top_20"><a href="espace_particulier.php?contenu=devis_histo">Retour � la page pr�c�dente</a></p>
					</div>';
				echo'
				<div id="espace_compte">
					'.$detail.'
			...
			else
			{
				...
				if ( $my->num($req)>0 )
				{
					...
					$liste.='</table><p class="margin_top_20"><a href="espace_particulier.php">Retour � la page pr�c�dente</a></p>';
				}
				else
				{
		...
		case 'devisa_encours' :
			if ( isset($_GET['idDevis']) )
			{
				if ( isset($_POST['nbr_est']) && $_POST['nbr_est']>1 )
				{
					$my->req('UPDATE ttre_achat_devis SET nbr_estimation="'.$my->net_input($_POST['nbr_est']).'" WHERE id='.$_GET['idDevis']);
				}
				$detail='';
				...
				$detail.='</table><p><br />
							<form method="post">
								Nombre d\'estimation : <input type="text" name="nbr_est" value="'.$infos_devis['nbr_estimation'].'" onKeyPress="return scanTouche(event)" /> 
				...
				$detail.='
				<p class="margin_top_20"><a href="espace_particulier.php?contenu=devisa_encours">Retour � la page pr�c�dente</a></p>
					';
				echo'
				<div id="espace_compte">
					'.$detail.'
			...
			else
			{
				...
				$alert='';
				if ( isset($_GET['valider']) ) $alert='<div id="note" class="success"><p>Votre devis a bien �t� valid�.</p></div><br />';
				if ( isset($_GET['supprimer']) ) $alert='<div id="note" class="success"><p>Votre devis a bien �t� supprim�.</p></div><br />';
				echo'
				'.$alert.'
				<div id="espace_compte">
					'.$liste.'
		...
		case 'devisa_histo' :
			if ( isset($_GET['idDevis']) )
			{
				...
				$detail.='</table><a id="various3" href="imp_cheque.php?idDevis='.$_GET['idDevis'].'&suite=ok">Aper�u avant imprimer</a>';
				...
				$detail.='
				<p class="margin_top_20"><a href="espace_particulier.php?contenu=devisa_histo">Retour � la page pr�c�dente</a></p>
					';
				echo'
				<div id="espace_compte">
					'.$detail.'
...
else
{
			...
			'.$alert.'
				...
				<div class="col-md-6 ">
					<div class="Login-part">
						<div class="title-sign-part">Vous avez d�j� un compte ?</div>
						<form action="espace_particulier.php?contenu=connexion" name="client_conn" method="post" class="" >
							<div class="form-login-part">
									...
									<p>Si vous avez oubli� votre mot de passe, <a href="espace_particulier.php?contenu=mdp_perdu" style="color:#000">cliquez ici</a>.</p>
							</div>
						</form>
				...
				<div class="col-md-6">
					<div class="Logup-part">
						<div class="title-sign-part">Vous �tes nouveau client ?</div>
						<div class="log-part">
							<p>Vous devez cr�er un compte pour pouvoir g�rer vos devis.</p>
							<p>Nous nous engageons � s�curiser vos informations et � les garder strictement confidentielles.</p>
							<a href="espace_particulier.php?contenu=inscription" class="">Cr�er un compte</a>
						</div>
					</div>
				</div>
	...
	else
	{
		...
		//<!--<div id="espace_compte">-->
		//<p class="padding_bottom_20">Bienvenue dans votre espace personnel, vous pouver depuis cette page g�rer toutes vos informations personnelles ainsi que vos commandes.</p>												
		echo'
			'.$alert.'		
				...
				<div class="col-md-4">
					<div class="espace-box1">
						...
						<a href="espace_particulier.php?contenu=modif_param">Modifier mes informations</a>
					</div>
				</div>
				...
				<div class="col-md-4">
					<div class="espace-box1">
						...
						<a href="espace_particulier.php?contenu=modif_mdp">Modifier mon mot de passe</a>
					</div>
				</div>
				...
				<div class="col-md-4">
					<div class="espace-box1">
						...
						<p>C\'est dans cette section que vous pouvez ajouter, modifier et supprimer vos adresses.</p>
						<a href="espace_particulier.php?contenu=adresse">G�rer mes adresses</a>
					</div>
				</div>	
				...
				<div class="col-md-4">
					<div class="espace-box1">
						...
						<h3>Historique devis immediat</h3>
						<p>C\'est dans cette section que vous pourrez consulter l\'historique de vos devis avec ench�re.</p>
						<a href="espace_particulier.php?contenu=devis_histo">historique</a>
					</div>
				</div>
				...
				<div class="col-md-4">
					<div class="espace-box1">
						...
						<h3>GESTION DE DEVIS AVEC visite d\'artisan</h3>
						<p>C\'est dans cette section que vous pourrez valider ou arr�ter votre devis avec achat imm�diat.</p>
						<a href="espace_particulier.php?contenu=devisa_encours">Devis en cours</a>
					</div>
				</div>    
				...
				<div class="col-md-4">
					<div class="espace-box1">
						...
						<h3>Historique de devis avec visite d\'artisan</h3>
						<p>C\'est dans cette section que vous pourrez consulter l\'historique de vos devis avec achat imm�diat.</p>
						<a href="espace_particulier.php?contenu=devisa_histo">historique</a>
					</div>
				</div>								
				...
				<div class="col-md-12">
					<div class="get-quote">						
						...
						<div class="quote-btns">
								<a href="devis.php" class="create">Cr�er votre devis</a>
								<a href="prix-travaux.php" class="get">Devis Imm�diat</a>
						</div>
					</div>
				</div>
				
				<p><a href="espace_particulier.php?contenu=deconnexion"> � Se d�connecter</a></p>		
			</div>
			';
	}
}
