...
if (!empty($_GET['action']))
	{$req_cat 	=  $my->req('SELECT * FROM ttre_panier WHERE id="'.$_GET['id'].'"');
	$cat		=  $my->arr($req_cat);
	switch($_GET['action'])
	{       case 'modifier' :
			if(!empty($_POST['description']))
			{
			$req_ajout_cat = $my->req("update ttre_panier
			...
			echo '<script>document.location.href="?contenu=panier" </script>';
			exit;
			}
			}
			else
			{
			$form = new formulaire('modele_1','?contenu=panier&action=modifier&id='.$_GET['id'],'<h2 class="titre_niv2">Modifier '.$cat['titre'].' :</h2>','','','sub','txt','','txt_obl','lab_obl');
...
{
echo '<h1>G&eacute;rer la page Panier</h1>';
echo '
...
$req_cat = $my->req('SELECT * FROM ttre_panier ');
...
<td class="bouton">
<a href="?contenu=panier&action=modifier&id='.$infos_cat['id'].'">
