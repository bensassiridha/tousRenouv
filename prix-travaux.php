	...
	switch ( $_GET['contenu'] )
	{
			...
			switch ( $_GET['action'] )
			{
				case 'ajouter' : 
					if( !isset($_SESSION['panier_trn']) )
					{
						...
						$_SESSION['panier_trn']['texte'] 	= array();
					}
					$indice = count($_SESSION['panier_trn']['id']) + 1;
					$prod=$_POST['id_prix'];$qua=$_POST['qte'];$rf=$_POST['type_rf'];
					$unite=$_POST['unite'];$piece=$_POST['piece'];$or_cat=$_POST['or_cat'];$texte=$_POST['texte'];
					...
					else
					{
						...
						if ( count($_SESSION['panier_trn']['id'])>0 )
						{
							...
							$_SESSION['panier_tmp']['texte'] 	= array();
							$indice = 1;
							...
							for ( $x=0;$x<=$nb_articles;$x++ )
							{
								...
								if ( $j==0 && $x==$cpt )
								{
									...
									$_SESSION['panier_tmp']['texte'][]=$texte; 
								...
								else
								{
									...
									array_push($_SESSION['panier_tmp']['texte'],$_SESSION['panier_trn']['texte'][$x]);
									$indice ++;
								}
							}
						...
						else
						{
							...
							$_SESSION['panier_trn']['texte'][]=$texte; 
						}
					}				
				...
				case 'modifier' :
					...
					{	
						...
						if ( isset($_POST['texte'.$_SESSION['panier_trn']['id'][$i]]) )
						{
							$_SESSION['panier_trn']['texte'][$i]=$_POST['texte'.$_SESSION['panier_trn']['id'][$i]];
						}
					}
		...
		case 'envoyer' :
			...
			{	
				...
				$texte = $_SESSION['panier_trn']['texte'][$i];
			...
			$message = '
				...
				<body style="background:white; margin:10px; font-family:Verdana, Arial, Helvetica, sans-serif; color:#000; font-size:14px;">
					<div id="corps" style="margin:0 auto; width:800px; height:auto;">
						...
						<p>Bonjour,</p>																
						<p>un nouveau devis avec enchere a été ajouté sur votre base de données par " '.html_entity_decode($cl['nom']).' '.html_entity_decode($cl['prenom']).' " 
...
$pageTitle = "Prix des travaux | Devis gratuit en ligne et sans aucun engagement"; 
...
if ( $my->num($reqCat)>0 )
{
	...
	while ( $resCat=$my->arr($reqCat) )
	{
		...
		$liCat.='<div class="col-md-2"><div class="row"><li><a href="prix-travaux.php?nomCat='.suppr_speciauxxx($resCat['titre']).'&cat='.$resCat['id'].'">'.$logo.' <br /> '.$resCat['titre'].'</a></li></div></div>';
	}
...
if ( isset($_GET['cat']) )
{
	...
	$resCat=$my->req_arr('SELECT * FROM ttre_categories WHERE id='.$_GET['cat'].' ORDER BY ordre ASC');
	...
	if ( $my->num($reqSCat)>0 )
	{
		...
		while ( $resSCat=$my->arr($reqSCat) )
		{
			...
			$liSCat.='<div class="col-md-2"><div class="row"><li><a href="prix-travaux.php?nomCat='.suppr_speciauxxx($resCat['titre']).'&cat='.$_GET['cat'].'&nomSCat='.suppr_speciauxxx($resSCat['titre']).'&scat='.$resSCat['id'].'">'.$logo.' <br /> '.$resSCat['titre'].'</a></li></div></div>';
		}
		echo'<div class="col-md-12">';
		echo '<ul id="menu_sub_cat" style="height:55px;">'.$liSCat.'</ul><div style="clear:both;"></div>';
		echo '</div>';
	}
}
...
{
	$resCat=$my->req_arr('SELECT * FROM ttre_categories WHERE id='.$_GET['cat'].' ORDER BY ordre ASC');
	$resSCat=$my->req_arr('SELECT * FROM ttre_categories WHERE id='.$_GET['scat'].' ORDER BY ordre ASC');
	...
	
	{
		...
		while ( $resPart=$my->arr($reqPart) )
		{
			if ( isset($_GET['part']) )
			{
				...
				{	
					echo'<p><strong>'.$resPart['titre'].' : <a href="prix-travaux.php?nomCat='.suppr_speciauxxx($resCat['titre']).'&cat='.$_GET['cat'].'&nomSCat='.suppr_speciauxxx($resSCat['titre']).'&scat='.$_GET['scat'].'&nomPart='.suppr_speciauxxx($resPart['titre']).'&part='.$resPart['id'].'">[ - ]</a></strong></p>';
					//----------------------------------- Partie Question ---------------------------
		...
		if ( $prixx>0 )
		{
			...
			echo'
				<form method="POST" action="?contenu=panier&action=ajouter">
					Nommer : <input type="text" name="piece" value="" /><br /><br />
					Commentaire : <textarea type="text" name="texte"/></textarea>
			...
			else 
			{
				...
				$contenu.='
					...
					<p class="padding_bottom_20 overflow_hidden">
						...
						<a href="prix-travaux.php?contenu=recapitulatif" class="boutons_persos_1 float_right">continuer</a>
					</p>			
						 ';
		...
		case 'recapitulatif' :
			...
			{	
				...
				$panier.='
						...
						<tr>
							<td>'.$nomPiece.'</td>		
							<td style="text-align:justify;">'.$nomProduit.'  <br /> Commentaire : <textarea>'.$texte.'</textarea></td>		
	...
	{	
		...
		$panier.='
				...
				<tr>
					...
					<td style="text-align:justify;">'.$nomProduit.' <br /> Commentaire : <textarea name="texte'.$clef.'">'.$texte.'</textarea></td>		
					<td>'.number_format($prixUnitaire, 2,'.','').' €</td>
